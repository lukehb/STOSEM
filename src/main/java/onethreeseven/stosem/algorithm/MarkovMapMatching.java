...
public class MarkovMapMatching {
    ...
    private boolean insert(List<QueryResult> potentialStates, GPXEntry gpxEntry, HMMTrellis hmm){
    ...
    private void subMapMatch(GPXIterator iter, MatchResult mr){
        ...
        while(iter.hasNext()){
            ...
            boolean success = insert(potentialStates, curEntry, hmm);
            if(success){
    ...
    private class RoadNode extends MarkovState {
        ...
        final TLongObjectHashMap<Path> pathsFrom;
        final long id;

        RoadNode(GPXExtension gpxExtension, int id) {
            this.id = id;
            this.gpxExtension = gpxExtension;
            this.pathsFrom = new TLongObjectHashMap<>();
        }
