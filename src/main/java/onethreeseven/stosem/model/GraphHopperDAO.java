package onethreeseven.stosem.model;
...
import onethreeseven.stosem.algorithm.MarkovMapMatching;
...
public class GraphHopperDAO implements GraphDAO {
     ...
     * @param fastMode If true, only keep the first best option when matching, this is quicker but more likely
     *                 to lead to inaccurate results.
    ...
    public MatchResult ghMapMatch(GPXFile gpxFile){
        ...
        AlgorithmOptions algoOpts = AlgorithmOptions.start()
                ...
                .weighting(new ShortestWeighting(new CarFlagEncoder()))
                .algorithm(Parameters.Algorithms.ASTAR)
        .build();

        MapMatching mapMatching = new MapMatching(gh, algoOpts);
     ...
     * @return a trajectory
     */
    public SpatialTrajectory matchesToTrajectory(List<EdgeMatch> matches){
    ...
    DoubleBuffer getAllEdgesWithinSector(final AbstractGeographicProjection projection, final LatLonBounds sector) {
        ...
        final double smallElevation = 10;
